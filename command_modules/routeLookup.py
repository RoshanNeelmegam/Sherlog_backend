...
def parsing_routing_table(contents, routes):
    ...
    biglist = re.findall(r'(\d+\.\d+\.\d+\.\d+\/\d+|vrf:.*)', contents, re.I)
    for vrf in vrfs_list_original:
        if vrf == 'Table_Ends_Here':
            break
        vrf_to_start_search = biglist.index(f'VRF: {vrf}') + 1
        vrf_to_end_search = biglist.index(f'VRF: {vrfs_list_original[vrfs_list_original.index(vrf) + 1]}')
